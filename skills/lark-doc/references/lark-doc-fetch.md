...
2. 如果目标是图片/文件素材，且用户只是想查看/预览，调用 lark-doc-media-preview（docs +media-preview）：
   ```bash
   lark-cli docs +media-preview --token "提取的token" --output ./preview_media
   ```
3. 如果用户明确要下载，或目标是 `<whiteboard token="..."/>`，调用 lark-doc-media-download（docs +media-download）：
   ```bash
